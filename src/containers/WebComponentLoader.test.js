...
describe("When isLoading prop is used", () => {
  beforeEach(() => {
    const middlewares = [];
    const mockStore = configureStore(middlewares);
    const initialState = {
  ...
  test("it respects isLoading prop when rendering", () => {
    render(
      <Provider store={store}>
        <CookiesProvider cookies={cookies}>
          <WebComponentLoader isLoading={true} />
        </CookiesProvider>
      </Provider>,
    );
    expect(screen.queryByText("webComponent.loading")).toBeInTheDocument();
  });
});
