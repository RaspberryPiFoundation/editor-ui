...
import {ProjectIndexPagination} from "../ProjectIndex/ProjectIndexPagination";
...
const ChatGPTModal = () => {
    ...
    console.log("CODE IS:", mainFileContent)
    console.log("ERROR IS:", errorMessage)

    const {loading, error, data, fetchMore} = useQuery(ERROR_EXPLANATION_QUERY, {
        ...
        <>
            ...
            >
                <div className='modal-content__header'>
                    <h2 className='modal-content__heading'>{t('chatGptModal.heading')}</h2>
                    <Button className='btn--tertiary' onClickHandler={closeModal} ButtonIcon={CloseIcon}/>
                </div>

                {!loading && data ?
                    <div style={{whiteSpace: "pre-wrap"}}>{data.errorExplanation.message}</div>
                    : null}
                {loading ?
                    <div><p>{t('chatGptModal.loading')}</p>
                ...
                <div className='modal-content__buttons'>
                    <Button className='btn--secondary' buttonText={t('filePane.renameFileModal.cancel')}
