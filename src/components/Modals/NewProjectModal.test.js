...
beforeEach(() => {
  createOrUpdateProject.mockImplementationOnce(() => Promise.resolve({ status: 200, data: {identifier: 'my-amazing-project'} }))
  const middlewares = []
  ...
  render(<Provider store={store}><MemoryRouter><div id='app'><NewProjectModal /></div></MemoryRouter></Provider>)
...
test('Creates python project correctly', async () => {
  ...
  expect(createOrUpdateProject).toHaveBeenCalledWith({...defaultPythonProject, name: 'My amazing project'}, 'my_token')
})
...
test('Creates HTML project correctly', async () => {
  ...
  await waitFor(() => fireEvent.click(saveButton))
  expect(createOrUpdateProject).toHaveBeenCalledWith({...defaultHtmlProject, name: 'My amazing project'}, 'my_token')
})
...
test('Navigates to new project', async () => {
  ...
  fireEvent.click(pythonOption)
  await waitFor(() => fireEvent.click(saveButton))
  expect(mockNavigate).toHaveBeenCalledWith('/ja-JP/projects/my-amazing-project')
})
