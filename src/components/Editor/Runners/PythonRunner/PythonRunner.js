...
const PythonRunner = () => {
  ...
  useEffect(() => {
    if (instructions?.steps) {
      setStep(instructions.steps[stepIndex]);
      setShowNextButton(stepIndex < instructions.steps.length - 1);
      setShowBackButton(stepIndex > 0);
    }
              ...
              <TabPanel key={1}>
                {instructions?.steps && (
                  <Tutorial
                    ...
                    // projectTitle={instructions?.content.title}
                    // projectSlug={"target-practice"}
