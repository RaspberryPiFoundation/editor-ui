...
- `preview`: Enable preview mode for Scratch projects — upload and download stay available, but saving and renaming are disabled (defaults to `false`). If `read_only` is also set, read-only behaviour takes precedence and upload is hidden.
