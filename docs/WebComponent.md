...
To have the web component be able to use the same React components as the site application there needed to be a separate start script for the component. This required being able to customise how the build was done, which is why the project has its own Vite config files rather than relying on a single shared build tool config.
